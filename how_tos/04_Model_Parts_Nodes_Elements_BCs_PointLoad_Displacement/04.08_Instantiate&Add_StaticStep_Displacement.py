...
### --- TAKE-AWAY 1: Instantiate a StaticStep. Add it to the problem -------------------------- ###

stp2 = StaticStep()
prb.add_step(stp2)

### --- TAKE-AWAY 2: Instantiate a GeneralDisplacement and add it to the step ----------------- ###

n2p1found = prt1.find_nodes_by_location(point=(2.0, 0.0, 0.0), distance=0.1)

dis1 = GeneralDisplacement(z=0.1)
stp2.add_displacement(nodes=n2p1found[0])

# ---------------------------- VIZ ---------------------------- #

### --- TAKE-AWAY 3: Have a summary of the Problem and visualize it --------------------------- ###

prb.summary()
#prb.show(draw_loads=0.0004,draw_bcs=0.2)
